        ...
        <div className="hidden bg-gray-800 mt-20 w-full  justify-center h-10 items-center md:flex gap-10">
          ...
          {(!user || user.role == "user") && (
            <>
              ...
              <CustomNavLink to="/men" text="Men" />
              <CustomNavLink to="/women" text="Women" />
              <CustomNavLink to="/electronics" text="Electronics" />
              {user && (
                <>
                  <CustomNavLink to="/orders" text="Orders" />
                  <CustomNavLink to="/profile" text="Profile" />
...
function CustomNavLink({ to, text, onClick }) {
  return (
    <NavLink
      ...
      onClick={onClick}
      className={({ isActive }) =>
        `text-gray-700 hover:text-blue-600 text-lg text-decoration-none transition duration-300 ${isActive ? 'font-bold' : ''
        }`
      }
    >
      {text}
